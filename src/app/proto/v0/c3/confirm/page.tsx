// src/app/proto/v0/c3/confirm/page.tsx
...
type SearchParams = {
  ...
  productId?: string;
  productPrice?: string;
  shippingId?: string; // c3も未選択はありうる（URLが壊れた/直リンク等）
  opt?: string | string[];
};
...
export default async function ProtoV0C3ConfirmPage({ searchParams }: Props) {
  ...
  // shippingId / opt は shipping ページから引き継ぐ
  const shippingId = sp.shippingId; // undefined のままありうる
  const options = toArray(sp.opt);

  // Shipping側と一致（express=150, normal=0, 未選択=0）
  const shippingPrice =
    shippingId === "express" ? 150 : shippingId === "normal" ? 0 : 0;

  // c3: warranty/newsletter（newsletterは0円）
  const optionsPrice =
    (options.includes("warranty") ? 300 : 0) +
    (options.includes("newsletter") ? 0 : 0);
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 space-y-6">
      <header className="space-y-1">
        <div className="text-xs text-gray-500">proto / v0 / c3</div>
      ...
      {/* c3: ConfirmSummary（構造はc2踏襲、DP差分は肯定文言） */}
      <ConfirmSummary
        isDP={isDP}
      ...
      <Link
        href={`/proto/v0/c3/shipping?${qp.toString()}`}
