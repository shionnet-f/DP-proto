// src/app/proto/v0/c1_dp2/shipping/page.tsx
...
const OPTIONS = [
  {
    id: "insurance",
    // control（通常表現）
    labelControl: "配送補償を付ける",
    // dp（not表現：不要ならチェック）
    labelDP: "配送補償を付けない（不要ならチェック）",
    priceYen: 300,
    desc: "破損・紛失に備えます",
  },
  {
    id: "giftwrap",
    labelControl: "ギフト包装を付ける",
    labelDP: "ギフト包装を付けない（不要ならチェック）",
    priceYen: 200,
    desc: "包装を追加します",
  },
] as const;
...
export default async function ProtoV0C1Dp2ShippingPage({
  searchParams,
}: Props) {
  ...
  const makeShippingHref = (shippingId: string) => {
    ...
    return `/proto/v0/c1_dp2/shipping?${qp.toString()}`;
  };
  ...
  const confirmHref = () => {
    ...
    if (selectedShippingId) qp.set("shippingId", selectedShippingId);
    for (const o of selectedOpts) qp.append("opt", o);
    return `/proto/v0/c1_dp2/confirm?${qp.toString()}`;
  };
    ...
    <main className="mx-auto max-w-6xl px-6 space-y-6">
      <header className="space-y-1">
        <h1 className="text-xl font-bold">
          カテゴリー1 / 配送・オプション（c1_dp2・{variant}）
        </h1>
        ...
        <div className="rounded-lg border bg-white p-4 space-y-3">
          ...
          <div className="space-y-2">
            {OPTIONS.map((o) => {
              ...
              const label = isDP ? o.labelDP : o.labelControl;
      ...
      <div className="flex items-center justify-between">
        <Link
          href={`/proto/v0/c1_dp2/products?variant=${variant}`}
