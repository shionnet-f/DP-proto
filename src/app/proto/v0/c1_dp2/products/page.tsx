// src/app/proto/v0/c1_dp2/products/page.tsx
// 商品ページ（分類1：視覚的強調DP） / Server Component
...
export default async function ProtoV0C1Dp2ProductsPage({
  searchParams,
}: Props) {
    ...
    <main className="mx-auto max-w-6xl px-6 space-y-6">
      <header className="space-y-2">
        <h1 className="text-xl font-bold">
          カテゴリー1 / 商品選択（c1_dp2・{variant}）
        </h1>

        {/* 課題指示（両条件で同一） */}
        <div className="rounded-lg border p-4 text-sm space-y-1 bg-white">
          <div className="font-semibold">
            課題：条件を満たす商品を購入してください
          </div>
          <div className="text-gray-700">
            条件：
            {CONDITION.minMah.toLocaleString()}mAh以上 / {CONDITION.maxWeightG}
            g以下 / ¥{CONDITION.maxPriceYen.toLocaleString()}以下
          </div>
        </div>
      </header>
          ...
          return (
            <div
              ...
              className={[
                ...
                isFeatured
                  ? "col-span-2 row-span-2 border-orange-300 bg-orange-50/50 shadow-lg p-4"
            ...
            >
              ...
              <div className="mt-4 flex justify-end">
                <Link
                  href={`/proto/v0/c1_dp2/shipping?variant=${variant}&productId=${p.id}&productPrice=${p.priceYen}`}
                  className={[
